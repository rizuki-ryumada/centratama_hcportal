$(function () {
    $('.tombolTambahData').on('click', function () {
        ...
        $("#target").val("");
		$("#icon").val("");
        $('.modal-body form').attr('action', 'http://192.168.23.51/hc_system/menu');
    ...
    $('.tampilModalUbah').on('click', function () {
        ...
        $('.modal-body form').attr('action', 'http://192.168.23.51/hc_system/menu/editmenu');
        // $('.modal-body form').attr('action', 'http://192.168.23.51/hc_uptothesky/menu/editmenu');
        const id = $(this).data('id');

        $.ajax({
            url: 'http://192.168.23.51/hc_system/menu/getMenuByID',
